...
fun ContentAddView(
    ...
    Column(
        ...
        Row(horizontalArrangement = Arrangement.Center, modifier = Modifier.padding(vertical = 16.dp)) {
            //INICIAR
            CircleButton(icon = painterResource(id = R.drawable.baseline_play_circle_24), enabled = !state.cronometroActivo) {
                cronometroVM.iniciar()
            }
            //PAUSAR
            CircleButton(icon = painterResource(id = R.drawable.baseline_pause_circle_filled_24), enabled = state.cronometroActivo) {
                cronometroVM.pausar()
            }

            //DETENER
            CircleButton(icon = painterResource(id = R.drawable.baseline_stop_circle_24), enabled = !state.cronometroActivo) {
                cronometroVM.detener()
            }

            //MOSTRAR / GUARDAR
            CircleButton(icon = painterResource(id = R.drawable.baseline_savings_24), enabled = state.showSaveButton) {
                cronometroVM.showTextField()
            }

        }

        if (state.showTextFiled){
